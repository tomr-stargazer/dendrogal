...
def make_quadrant_lbv_map(cloud_catalog, dendgrogram, contour_select=True, alignment='lv'):
    ...
    if contour_select:
        dv.hub.select(1, cloud_selection, subtree=False)
        dv.hub.select(2, None)
    ...
    iv.ax_lv.set_xlabel(r"Galactic longitude $l$ (deg)")
    iv.ax_lv.set_ylabel(r"Velocity $v_{LSR}$ (km s$^{-1}$)")            
